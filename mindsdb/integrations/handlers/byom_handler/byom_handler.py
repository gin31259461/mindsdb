...
class BYOMHandler(BaseMLEngine):
    ...
    def get_model_engine_version(self) -> int:
        ...
        """
        engine_version = (
            self.model_storage.get_info()["learn_args"]
            .get("using", {})
            .get("engine_version")
        )
        engine_version = BYOMHandler.normalize_engine_version(engine_version)
    ...
    def _get_model_proxy(self, version=None):
        ...
        try:
            ...
            modules_str = self.engine_storage.fileStorage.file_get(
                f"modules{version_mark}"
            )
    ...
    def create_engine(self, connection_args):
        ...
        self.engine_storage.fileStorage.file_set(
            "modules", requirements_path.read_bytes()
        )
    ...
    def update_engine(self, connection_args: dict) -> None:
        ...
        if (
            isinstance(engine_connection_args, dict) is False
            or "handler_version" not in engine_connection_args
        ):
        ...
        new_version = str(
            max([int(x) for x in engine_connection_args["versions"].keys()]) + 1
        )
        ...
        self.engine_storage.fileStorage.file_set(
            f"code_{new_version}", code_path.read_bytes()
        )

        self.engine_storage.fileStorage.file_set(
            f"modules_{new_version}", requirements_path.read_bytes()
        )
        self.engine_storage.fileStorage.push()
    ...
    def finetune(
        ...
        try:
            ...
            model_state = model_proxy.finetune(
                df, model_state, args=args.get("using", {})
            )
...
class ModelWrapperSafe:
    ...
    def prepare_env(self, modules, engine_id, engine_version: int):
        try:
            ...
            logger.info(
                "Can't create virtual environment. venv module should be installed"
            )
    ...
    def install_modules(self, modules, pip_cmd):
        ...
        for module in modules:
            ...
            if p.returncode != 0:
                raise Exception(
                    f"Problem with installing module {module}: {p.stderr.read()}"
                )
    ...
    def _run_command(self, params):
        ...
        wrapper_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "proc_wrapper.py"
        )
        p = subprocess.Popen(
